...
import {COUNTRY_NAMES,destinationLabel,dateLabel} from '@/lib/types';
...
export default function RecurringProgrammes({records}:{records:RecurringRecord[]}){
 ...
 const countries=[...new Set(records.flatMap(record=>[record.country,...(record.destinationCountries||[])]))].sort((a,b)=>(COUNTRY_NAMES[a]||a).localeCompare(COUNTRY_NAMES[b]||b,'es'));
 ...
 return <section aria-label="Directorio de programas recurrentes">
  <div className="recurring-filters" id="programas-filtros">
   ...
   <label className="filter-choice"><span>Destino de la edición documentada</span><select value={filters.country} onChange={e=>change('country',e.target.value)}><option value="all">Todos los países</option>{countries.map(country=><option value={country} key={country}>{COUNTRY_NAMES[country]||country}</option>)}</select></label>
  </div>
  ...
  {results.length===0?<div className="recurring-empty"><h2>No hay programas con esta combinación.</h2><p>Prueba otro país o nivel de acceso, o elimina los filtros.</p><button type="button" className="primary-button" onClick={()=>setFilters({...recurringDefaults})}>Ver todos los programas</button></div>:<div className="recurring-list">{results.slice((page-1)*pageSize,page*pageSize).map(record=><article className="recurring-item" key={record.id}>
   <div className="recurring-identity"><p className="eyebrow">{RECURRENCE_CATEGORIES[record.recurrence.category]} · {destinationLabel(record)}</p><h2><Link href={opportunityHref(record.id,returnHref)}>{record.title}<ArrowUpRight size={19} aria-hidden="true"/></Link></h2><p className="institution">{record.institution}</p><p className="recurring-entry">{record.entry}</p><div className="recurring-links"><SaveOpportunityButton record={{id:record.id,title:record.title,institution:record.institution}}/><Link className="text-link" href={opportunityHref(record.id,returnHref)}>Requisitos, costes y evidencia <ArrowRight size={15} aria-hidden="true"/></Link><a href={record.url} target="_blank" rel="noreferrer">Fuente oficial <ArrowUpRight size={15} aria-hidden="true"/><span className="sr-only"> (se abre en otra pestaña)</span></a></div></div>
