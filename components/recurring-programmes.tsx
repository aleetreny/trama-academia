...
export default function RecurringProgrammes({records}:{records:RecurringRecord[]}){
 ...
 return <section aria-label="Directorio de programas recurrentes">
  ...
  {results.length===0?<div className="recurring-empty"><h2>No hay programas con esta combinación.</h2><p>Prueba otro país o nivel de acceso, o elimina los filtros.</p><button type="button" className="primary-button" onClick={()=>setFilters({...recurringDefaults})}>Ver todos los programas</button></div>:<div className="recurring-list">{results.slice((page-1)*pageSize,page*pageSize).map(record=><article className="recurring-item" key={record.id}>
   <div className="recurring-identity"><p className="eyebrow">{RECURRENCE_CATEGORIES[record.recurrence.category]} · {COUNTRY_NAMES[record.country]||record.country}</p><h2><Link href={'/oportunidad/'+record.id}>{record.title}<ArrowUpRight size={19} aria-hidden="true"/></Link></h2><p className="institution">{record.institution}</p><p className="recurring-entry">{record.entry}</p><div className="recurring-links"><Link className="text-link" href={'/oportunidad/'+record.id}>Requisitos, costes y evidencia <ArrowRight size={15} aria-hidden="true"/></Link><a href={record.url} target="_blank" rel="noreferrer">Fuente oficial <ArrowUpRight size={15} aria-hidden="true"/><span className="sr-only"> (se abre en otra pestaña)</span></a></div></div>
