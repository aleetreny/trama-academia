...
export function OpportunityCard({record:r,selected=false,onCompare,compareDisabled=false,loading=false,unavailable=false,research,subject,returnHref}:{record:Pick<Opportunity,'id'|'kind'|'status'|'deadline'|'lastError'|'verifiedAt'|'institution'|'title'|'city'|'country'|'stage'|'fields'|'funding'|'entry'|'recurrence'>;loading?:boolean;unavailable?:boolean;research?:ResearchInstitution;subject?:string;selected?:boolean;compareDisabled?:boolean;returnHref?:string;onCompare?:(id:string)=>void}){const status=statusOf(r);const href=opportunityHref(r.id,returnHref);return <article className="opportunity-card">
