...
import {syncObservations} from './sync-observations.mjs';
import {isEuropeanDestination} from '../harvest/programme-geography.mjs';
...
if(data.records.some(r=>!isEuropeanDestination(r)))throw new Error('Non-European or unverified campus destination; audit before publishing');
...
let observationSync=null;
let observationCache;
try{observationCache=await fs.readFile('.cache/observations.json','utf8');}catch(e){if(e.code!=='ENOENT')throw e;}
if(observationCache!==undefined){
 const {runId,observations}=JSON.parse(observationCache);
 observationSync=await syncObservations(sql,observations,{runId,fallbackRunId:data.run?.id});
}
const count=await sql`SELECT count(*)::int AS count FROM opportunities`;
console.log(JSON.stringify({published:data.records.length,databaseRecords:count[0].count,generatedAt:data.generatedAt,observations:observationSync}));
