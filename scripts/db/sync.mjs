...
try{const {runId,observations}=JSON.parse(await fs.readFile('.cache/observations.json','utf8'));for(let i=0;i<observations.length;i+=100)await sql.transaction(observations.slice(i,i+100).map(o=>sql`INSERT INTO observations(id,run_id,url,checked_at,http_status,content_hash,outcome) VALUES(${createHash('sha256').update(o.url+'|'+o.checkedAt).digest('hex')},${runId||data.run.id},${o.url},${o.checkedAt},${o.httpStatus},${o.hash},${o.outcome}) ON CONFLICT(id) DO NOTHING`));}catch(e){if(e.code!=='ENOENT')throw e;}
