...
import {JOBBNORGE_FEED,parseJobbnorgeFeed,verifyJobbnorge} from './jobbnorge.mjs';
...
async function worker(){while(next<data.records.length){const old=data.records[next++];old.status=effectiveStatus(old);if(old.kind!=='position'||old.status==='closed'||Date.now()-Date.parse(old.verifiedAt||0)<6*3600000)continue;
 ...
 try{let fresh;if(source.adapter==='jobbnorge'){
  ...
  const {page,jobs}=await jobbnorgeFeed;const job=jobs.find(j=>String(j.id)===old.evidence?.sourceRecordId);if(!job)throw new Error('feed_missing_offer');fresh=await verifyJobbnorge(job,source,page);
 ...
 catch(e){if(source.adapter!=='jobbnorge'&&['http_404','http_410'].includes(e.message)){old.status='closed';old.closedAt=new Date().toISOString();counts.closed++;}else{old.lastError=e.message;old.lastAttemptAt=new Date().toISOString();old.status=effectiveStatus(old);counts.failed++;}}
}}
