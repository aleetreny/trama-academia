...
export function applyReviewedCorrection(record,text,reviewed=corrections){
 const correction=reviewed.find(c=>c.id===record.id);if(!correction)return record;
 ...
 return {...record,...structuredClone(correction.patch),evidence:{...record.evidence,checks:[...(record.evidence?.checks||[]),'Reviewed source correction: '+correction.reviewedAt]}};
}
