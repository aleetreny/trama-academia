...
async function worker(){while(next<seeds.length){const s=seeds[next++];try{
 const page=await getPage(s.url);const $=load(page.body);$('script,style,nav,header,footer').remove();const text=clean(($('main').length?$('main'):$('body')).text());
