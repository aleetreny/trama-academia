...
La edición publicada tras la revisión contiene **3.637 registros, 3.201 fuentes, 3.186 semillas y 6.360 instituciones**. Son **264 altas** sobre la referencia anterior: 251 programas de máster verificados, cinco escuelas de verano y ocho vacantes de la actualización semanal revisadas antes de integrar. El directorio recurrente agrupa 19 fichas y las guías cubren 31 países. El [informe de revisión](review-2026-09-21.md) documenta los denominadores, correcciones y límites.

**Revisión completada, sincronizada y publicada.** Pasan 219 pruebas Node, dos PDF, tipos y lint sin avisos. El build de 3.637 fichas comprueba 88.950 referencias internas y los presupuestos. La revisión de navegador confirma filtros, comparación, teclado, recuperación de errores, móvil y tableta. Las 13 correcciones de fichas existentes están verificadas. La [actualización semanal 35586680322](https://github.com/aleetreny/trama-academia/actions/runs/35586680322) terminó correctamente y se integró, conservando la cola y el registro. Neon confirma 3.637 fichas activas y 608 observaciones añadidas sin sobrescribir las 124.545 existentes. El [CI y despliegue 35597393969](https://github.com/aleetreny/trama-academia/actions/runs/35597393969) publicó el commit funcional `1fece6c21c02a4af8e1a4fec8e261d802504cf79`; se comprobaron su `build-info.json` y sus rutas públicas. El [informe](review-2026-09-21.md#validación-y-publicación) registra el cierre y sus límites.
...
4. [Revisión integral del 21 de septiembre](review-2026-09-21.md) y [guía de presentación](presentation.md): alcance cerrado, comprobaciones realizadas y límites del producto. Si se solicita otra ampliación, contrastar la cola y los candidatos con el catálogo actual antes de incorporar nada.
...
- **Más cobertura, cuando se solicite:** `data/institution-crawl.summary.json` describe una cola reanudable de 438.450 trabajos, con 111.932 lecturas y 171.731 pendientes en esa instantánea. El archivo histórico `work/discovery/candidates-after-crawl-28.json` contiene 21.640 candidatos, 3.305 prioritarios y 41 retenidos anteriores a esta revisión. Son enlaces por revisar, no oportunidades nuevas confirmadas; pueden solaparse con el catálogo y con las últimas incorporaciones. Revisar por país e institución y deduplicar antes de integrar. La cobertura europea sigue incompleta.
...
- **Sincronización histórica:** esta edición quedó sincronizada y verificada. Se han añadido inserción por lotes de observaciones faltantes, conservación del identificador de origen y escritura atómica del archivo local. Las comprobaciones parciales de programas guardan checkpoints y no acreditan un barrido completo. En futuras ejecuciones, comprobar el resultado real; una operación preparada o interrumpida no significa que todos los datos hayan llegado a Neon. Seguir la [guía de operación](operations.md) y evitar repetir toda la historia sin necesidad.
...
Para cambios funcionales: `npm test`, `npm run lint`, `npm run typecheck`, `npm run build` y las pruebas PDF cuando corresponda. El build comprueba integridad de fichas y presupuestos. `npm run dev` sirve el proyecto bajo `/trama-academia/`; `npm start` permite revisar la exportación en el puerto 4173. La web no necesita credenciales para construirse.

La revisión está cerrada con las evidencias del [informe](review-2026-09-21.md#validación-y-publicación). Los artefactos locales están en `work/revision-integral-2026-09-21/`, incluido el checkpoint de publicación. En futuras entregas, comprobar CI, `build-info.json` y las rutas afectadas en la web pública. Para cambios exclusivamente documentales basta revisar referencias y diff; no hace falta recolectar fuentes ni modificar el catálogo.
