...
Un programa concreto puede incorporarse antes de resolver el alcance geográfico de toda su institución. Para ello `geography` identifica el campus, la ciudad, un área europea revisada y las fuentes; `campus-programme` enlaza la ficha o currículo con la unidad docente, y `campus` comprueba literalmente su ubicación. Istanbul por sí sola no basta. Se conservan las URLs, fechas, hashes y frases comprobadas en `evidence.geography`. El verificador, la auditoría y la publicación en Neon rechazan una mera etiqueta sin esa evidencia. Las zonas revisadas inicialmente son Moscú, San Petersburgo y determinados campus/distritos europeos de Estambul; agregar otra zona requiere revisión. Como ejemplo del riesgo de generalizar, la [guía institucional de ITU](https://global.itu.edu.tr/docs/librariesprovider33/default-document-library/%C4%B1tu_katalog5de08c17df724775901094209c5da1f2.pdf?sfvrsn=293b17db_0) sitúa Tuzla en la parte asiática. Ninguna comprobación de un programa concede un tier institucional ni abre la selección automática de vacantes para todo el país.
...
La cola comprimida `institution-crawl.json.gz` contiene un manifiesto y los metadatos de las ejecuciones; las tareas y sus pruebas se conservan en `institution-crawl.parts/`, en partes de hasta 8 MiB sin comprimir. Cada parte tiene una huella SHA-256 y comprobaciones de tamaño y recuento. El lector admite también las instantáneas anteriores de un solo archivo y falla ante datos corruptos o partes ausentes, sin reiniciar la historia. Las escrituras preparan las partes antes de sustituir el manifiesto de forma atómica. `institution-crawl.summary.json` permite revisar los recuentos en Git. La migración comprobó igualdad exacta de las 396.985 tareas y de todos los metadatos, sin eliminar historia. Guarda una tarea por institución y URL canónica. Se parte de webs oficiales de ROR y de las fuentes localizadas mediante revisión. Cada enlace nuevo conserva la página desde la que se observó; no se inventan rutas institucionales. Los parámetros que identifican contenido se conservan y se eliminan los de seguimiento.
...
La descarga se detiene antes de agotar la cuota gratuita de OpenAlex y tiene un límite de 600 peticiones de red por ejecución. Las respuestas se reutilizan durante siete días; el marcador de cuota evita nuevos intentos durante el mismo día UTC cuando quedan menos de 20 créditos. Repetir el comando continúa desde la caché. Las identidades se reutilizan por ROR con su referencia y fecha, aunque al ampliar el universo cambien los lotes. `--subjects=ml,statistics,applied-math` permite priorizar disciplinas pendientes sin borrar las ya completas; cada disciplina conserva el alcance geográfico de sus propias consultas. Solo se publican disciplinas con los cuatro agregados completos. La actualización semanal de fuentes no recalcula estos indicadores.

La edición completada el **21 de septiembre de 2026** contiene las cuatro disciplinas: **850** instituciones elegibles en informática, **548** en IA y aprendizaje automático, **142** en estadística y probabilidad y **89** en matemáticas aplicadas. Estas cohortes se solapan y no se suman como instituciones únicas. Cada disciplina usa sus propios cuatro agregados, sin extrapolar los de informática. Las consultas de las cuatro incluyen ahora los 58 códigos del ámbito de búsqueda, incluida la ampliación académica de Armenia, Azerbaiyán y Georgia; la geografía del campus sigue siendo un filtro independiente para países transcontinentales. Al ampliar el ámbito y actualizar sus agregados, informática pasa de 844 a 850 instituciones elegibles. Los percentiles pueden cambiar cuando cambia la cohorte. Las referencias conservan sus fechas originales de comprobación, incluso cuando se reutilizan respuestas en caché.
