...
export type SearchRecord=Pick<Opportunity,'id'|'title'|'institution'|'country'|'funderCountry'|'destinationCountries'|'city'|'kind'|'stage'|'eligibleStages'|'recurrence'|'fields'|'status'|'verifiedAt'|'deadline'|'languages'|'lastError'>&{funding:{kind:string};researchInstitutionId:string|null;detailPath:string};
...
export type Filters={stage:string;q:string;country:string;funderCountry:string;field:string;kind:string;status:string;subject:string;tier:string;funding:string;language:string;closing:string;sort:string;page:number};
export const defaults:Filters={stage:'all',q:'',country:'all',funderCountry:'all',field:'all',kind:'all',status:'current',subject:'cs',tier:'all',funding:'all',language:'all',closing:'all',sort:'prestige',page:1};
const keys:Record<keyof Filters,string>={stage:'etapa',q:'q',country:'pais',funderCountry:'financiador',field:'campo',kind:'tipo',status:'vigencia',subject:'disciplina',tier:'tier',funding:'financiacion',language:'idioma',closing:'cierre',sort:'orden',page:'pagina'};
const choices:Partial<Record<keyof Filters,readonly string[]>>={stage:['all',...STAGES.map(s=>s.id)],country:['all',...Object.keys(COUNTRY_NAMES)],funderCountry:['all','unknown',...Object.keys(COUNTRY_NAMES)],field:['all',...FIELDS],kind:['all','programme','master-programme','position'],status:['current','all','open','rolling','listed','programme','unverified','closed'],subject:SUBJECTS.map(s=>s[0]),tier:['all','ranked','T1','T1T2','T2','T3','T4','unranked'],funding:['all','salary','scholarship','waiver','grant','mixed','unconfirmed'],language:['all','english','spanish','french','german'],closing:['all','7','30','90'],sort:['prestige','impact','volume','deadline','recent','institution','title']};
...
export function selectRecords(data:SearchData,f:Filters,now=Date.now()){
 ...
 const rows=data.records.filter(r=>{
  ...
  if(f.country!=='all'&&r.country!==f.country&&!r.destinationCountries?.includes(f.country)||f.field!=='all'&&!r.fields.includes(f.field))return false;
  if(f.funderCountry!=='all'&&(f.funderCountry==='unknown'?Boolean(r.funderCountry):r.funderCountry!==f.funderCountry))return false;
  if(terms.length){const text=normaliseSearch([r.title,r.institution,r.city,COUNTRY_NAMES[r.country],...(r.destinationCountries||[]).map(country=>COUNTRY_NAMES[country]),...r.fields].join(' ')),theme=normaliseSearch([r.title,...r.fields].join(' '));if(!terms.every(term=>term.pattern?term.pattern.test(term.thematic?theme:text):text.includes(term.text)))return false;}
