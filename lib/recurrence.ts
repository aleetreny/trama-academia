...
export type RecurringRecord=Pick<Opportunity,'id'|'title'|'institution'|'country'|'destinationCountries'|'url'|'stage'|'eligibleStages'|'entry'|'verifiedAt'|'lastError'> & {recurrence:ProgrammeRecurrence};
...
export function selectRecurringRecords(records:RecurringRecord[],filters:RecurringFilters){
 ...
 return records.filter(record=>{
  ...
  if(filters.country!=='all'&&record.country!==filters.country&&!record.destinationCountries?.includes(filters.country))return false;
